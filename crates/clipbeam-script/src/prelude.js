...
//   * 真正的编码转换交给 Rust 原语 `globalThis.__clipbeam`（见 src/bindings/text.rs），
...
(() => {
	...
	const prim = globalThis.__clipbeam;
	if (!prim) {
		throw new Error("clipbeam prelude: 内部对象 __clipbeam 未注册（运行时初始化顺序有问题）");
	}
