//! TypeScript 转译的集成测试（搬运自 clipbeam_js，断言保持一致）。
//!
...
use clipbeam_script::{ts, ScriptRuntime};

/// 把源码写进临时 `.ts` 文件，返回路径（转译器需要真实扩展名来判定语法）。
fn write_ts(name: &str, source: &str) -> PathBuf {
    let mut path = std::env::temp_dir();
    path.push(format!("clipbeam-ts-{}-{name}", std::process::id()));
...
fn allows_top_level_await_without_imports() {
    let js = transpile(
        "tla.ts",
        r#"
        const bytes: ArrayBuffer = await $.file("Cargo.toml");
        bytes.byteLength;
        "#,
    )
...
async fn end_to_end_typescript_script_runs() {
    let payload = b"typescript end to end\n".to_vec();
    let mut data_path = std::env::temp_dir();
    data_path.push(format!("clipbeam-ts-{}-e2e.bin", std::process::id()));
    std::fs::write(&data_path, &payload).expect("写入数据文件失败");

    // 路径先转成 JS 字面量再嵌进脚本，避免 format! 嵌套
    let path_literal = format!("{:?}", data_path.to_str().unwrap());
    let source = format!(
        r#"
        interface Result {{ size: number }}
        const bytes: ArrayBuffer = await $.file({path_literal});
