...
// 因此不需要 DOM：
//
//   1. hover `Clipbeam` / `$.typeStr` / 变量 → 有类型与 JSDoc；
 ...
 */
const SCRIPT = `const bytes = await $.file('README.md')
const digest = $.md5(bytes)
Clipbeam.typeStr('hello', 10)
...
describe('hover（悬停提示）', () => {
  it('悬停 Clipbeam 给出它的类型', async () => {
    const info = await quickInfo(NAME, SCRIPT, offsetOf('Clipbeam.typeStr') + 2)
    expect(info, 'Clipbeam 上应当有 hover 信息').not.toBeNull()
    expect(info?.signature).toContain('Clipbeam')
  })

  it('悬停 $.typeStr 给出签名与 JSDoc', async () => {
    // 落在 `typeStr` 中间
    const info = await quickInfo(NAME, SCRIPT, offsetOf('typeStr') + 3)
    expect(info, '$.typeStr 上应当有 hover 信息').not.toBeNull()
    expect(info?.signature).toContain('typeStr')
    // 声明文件里的 @param/JSDoc 应当被带出来
    expect(info?.signature).toContain('text: string')
    expect(info?.documentation.length, 'typeStr 应当带 JSDoc 说明').toBeGreaterThan(0)
  })
...
describe('completions（代码补全）', () => {
  it('clipbeam. 之后列出能力', async () => {
    ...
    for (const expected of ['file', 'sleep', 'md5', 'zstd', 'typeStr', 'confirm']) {
  ...
  it('$. 之后列出同样的能力（$ 是 Clipbeam 的别名）', async () => {
    ...
    expect(names).toContain('typeStr')
  })
  ...
  it('arraybuffer 变量之后列出它的成员（类型推断生效）', async () => {
    const source = `const bytes = await $.file('README.md')\nbytes.\n`
  ...
  it('能力补全带签名详情（来自声明文件）', async () => {
    const entries = await completions(NAME, SCRIPT, offsetAfter('Clipbeam.'))
    const typeStr = (entries ?? []).find(entry => entry.name === 'typeStr')
    expect(typeStr, '应当能补全出 typeStr').toBeDefined()
    const details = typeStr?.getDetails()
    expect(details?.signature).toContain('typeStr')
...
describe('signatureHelp（参数信息）', () => {
  it('在 zstd( 里给出签名与参数', async () => {
    ...
    expect(info?.label).toContain('chunkSize')
...
describe('typecheck（诊断，防回归）', () => {
  it('类型不匹配仍然报错', async () => {
    const bad = `await $.md5('不是 ArrayBuffer')\n`
